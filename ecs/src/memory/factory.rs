...
impl Factory {
    ...
    fn add_get_pool_or_get<T: AnyComponent + 'static>(&mut self) -> &mut ComponentPool<T> {
        let id = T::id();
    ...
    pub fn add_get_or_get_component<T: AnyComponent + 'static>(&mut self, entity: &Entity, value: T) -> &mut T {
        let pool = self.add_get_pool_or_get::<T>();
    ...
    pub fn try_remove_component(&mut self, entity: &Entity, id: Component) -> bool {
        return match self.try_get_pool_any_mut(id) {
